---
id: start
title: Getting started with the API
sidebar_label: Getting started
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

## Repositories
### Releases
Use this repository to work with the release version of the API (recommended)

<Tabs
  groupId="repositories"
  defaultValue="maven"
  values={[
  {label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"}
]}>
  <TabItem value="maven">

  ```xml
  <repository>
      <id>cloudnet-releases</id>
      <url>https://repo.cloudnetservice.eu/repository/releases/</url>
  </repository>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  maven { url 'https://repo.cloudnetservice.eu/repository/releases/' }
  ```

  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  maven("https://repo.cloudnetservice.eu/repository/releases/")
  ```

  </TabItem>
  </Tabs>

  ### Snapshots
  Use this repository to work with a snapshot version of the API (**Only** recommended for development!)

  <Tabs
  groupId="repositories"
  defaultValue="maven"
  values={[
  {label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"}
  ]}>
  <TabItem value="maven">

  ```xml
  <repository>
      <id>cloudnet-snapshots</id>
      <url>https://repo.cloudnetservice.eu/repository/snapshots/</url>
  </repository>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  maven { url 'https://repo.cloudnetservice.eu/repository/snapshots/' }
  ```

  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  maven("https://repo.cloudnetservice.eu/repository/snapshots/")
  ```

  </TabItem>
</Tabs>

## Dependencies
You have to replace `${VERSION}` with the version which you want to use. (e.g. `3.3.0-RELEASE` or `3.4.0-SNAPSHOT`)

### cloudnet
CloudNet application for modules (NOT AVAILABLE FOR PLUGINS!)
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet org="de.dytanic.cloudnet" name="cloudnet" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>

### cloudnet-driver
CloudNet driver for plugins and modules
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet-driver>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet-driver</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet-driver>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet-driver:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet-driver:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet-driver" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet-driver org="de.dytanic.cloudnet" name="cloudnet-driver" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet-driver', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet-driver "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet-driver:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>

### cloudnet-wrapper-jvm
CloudNet wrapper for plugins
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet-wrapper-jvm>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet-wrapper-jvm</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet-wrapper-jvm>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet-wrapper-jvm:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet-wrapper-jvm:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet-wrapper-jvm" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet-wrapper-jvm org="de.dytanic.cloudnet" name="cloudnet-wrapper-jvm" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet-wrapper-jvm', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet-wrapper-jvm "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet-wrapper-jvm:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>

### cloudnet-bridge
CloudNet bridge module for plugins and modules
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet-bridge>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet-bridge</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet-bridge>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet-bridge:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet-bridge:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet-bridge" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet-bridge org="de.dytanic.cloudnet" name="cloudnet-bridge" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet-bridge', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet-bridge "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet-bridge:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>

### cloudnet-cloudperms
CloudNet CloudPerms module for plugins and modules
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet-cloudperms>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet-cloudperms</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet-cloudperms>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet-cloudperms:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet-cloudperms:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet-cloudperms" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet-cloudperms org="de.dytanic.cloudnet" name="cloudnet-cloudperms" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet-cloudperms', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet-cloudperms "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet-cloudperms:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>

### cloudnet-syncproxy
CloudNet SyncProxy module for plugins (proxy) and modules
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet-syncproxy>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet-syncproxy</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet-syncproxy>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet-syncproxy:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet-syncproxy:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet-syncproxy" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet-syncproxy org="de.dytanic.cloudnet" name="cloudnet-syncproxy" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet-syncproxy', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet-syncproxy "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet-syncproxy:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>

### cloudnet-signs
CloudNet signs module for plugins and modules
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet-signs>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet-signs</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet-signs>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet-signs:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet-signs:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet-signs" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet-signs org="de.dytanic.cloudnet" name="cloudnet-signs" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet-signs', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet-signs "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet-signs:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>

### cloudnet-npcs
CloudNet NPCs module for plugins and modules
<Tabs
  groupId="dependencies"
  defaultValue="maven"
  values={[{label: "Maven", value: "maven"},
  {label: "Gradle", value: "gradle-groovy"},
  {label: "Gradle Kotlin DSL", value: "gradle-kotlin"},
  {label: "SBT", value: "sbt"},
  {label: "Ivy", value: "ivy"},
  {label: "Grape", value: "grape"},
  {label: "Leiningen", value: "leiningen"},
  {label: "Buildr", value: "buildr"}
]} >

  <TabItem value="maven">

  ```xml
  <cloudnet-npcs>
      <groupId>de.dytanic.cloudnet</groupId>
      <artifactId>cloudnet-npcs</artifactId>
      <version>${VERSION}</version>
      <scope>provided</scope>
  </cloudnet-npcs>
  ```

  </TabItem>
  <TabItem value="gradle-groovy">

  ```groovy
  implementation 'de.dytanic.cloudnet:cloudnet-npcs:${VERSION}'
  ```
  </TabItem>
  <TabItem value="gradle-kotlin">

  ```kotlin
  implementation("de.dytanic.cloudnet:cloudnet-npcs:${VERSION}")
  ```

  </TabItem>
  <TabItem value="sbt">

  ```
  libraryDependencies += "de.dytanic.cloudnet" % "cloudnet-npcs" % "${VERSION}"
  ```

  </TabItem>
  <TabItem value="ivy">

  ```xml
  <cloudnet-npcs org="de.dytanic.cloudnet" name="cloudnet-npcs" rev="${VERSION}"/>
  ```

  </TabItem>
  <TabItem value="grape">

  ```
  @Grapes(
      @Grab(group='de.dytanic.cloudnet', module='cloudnet-npcs', version='${VERSION}')
  )
  ```

  </TabItem>
  <TabItem value="leiningen">

  ```
  [de.dytanic.cloudnet/cloudnet-npcs "${VERSION}"]
  ```

  </TabItem>
  <TabItem value="buildr">

  ```
  'de.dytanic.cloudnet:cloudnet-npcs:jar:${VERSION}'
  ```

  </TabItem>
</Tabs>